...
func (an *appNats) Configure() error {
	...
	an.logger.Infoln("Connection to NATS service...")
...
func (an *appNats) setupConnOptions(opts []nats.Option) []nats.Option {
	...
	opts = append(opts, nats.DisconnectErrHandler(func(nc *nats.Conn, err error) {
		an.logger.Infof("Disconnected due to: %s, will attempt to reconnect for %.0fm", err, totalWait.Minutes())
	}))
	opts = append(opts, nats.ReconnectHandler(func(nc *nats.Conn) {
		an.logger.Infof("Recoonected [%s]", nc.ConnectedUrl())
	}))
...
func (an *appNats) Subscribe(ctx context.Context, channel pubsub.Topic) (<-chan *pubsub.Message, func()) {
